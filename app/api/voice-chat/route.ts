...
// ✅ IMPROVED: Language-specific system prompts with better rules and critical context
function getSystemPrompt(doctorName: string, specialty: string, language: string, conversationStage: 'greeting' | 'ongoing'): string {
  const languageInstructions: Record<string, any> = {
    yoruba: {
      greeting: `You are **${doctorName}**, a **${specialty}** specialist starting a NEW medical consultation in YORUBA ONLY.
...
Be a caring Nigerian doctor speaking naturally in Yoruba.`,
      ...
      // 💡 CORRECTION: Added CRITICAL CONTEXT to prevent misinterpretation (e.g., orífi)
      ongoing: `You are **${doctorName}**, a **${specialty}** specialist conducting an ONGOING medical consultation in YORUBA ONLY.
...
***CRITICAL CONTEXT: The patient just responded to your initial questions. Interpret their last message as their primary symptom (e.g., headache, fever). Respond with relevant, diagnostic follow-up questions. DO NOT get confused by similar sounding words (like misinterpreting 'headache' for 'hair' or 'stool').***
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 💡 CORRECTION: Correctly determine if this is the very first turn.
    const isNewConsultation = existingConversation.length === 0;
    const conversationStage = isNewConsultation ? 'greeting' : 'ongoing';
    ...
    // 💡 CORRECTION: Set up messages for AI based on consultation stage
    let messagesForAI = existingConversation.map((msg) => ({
      role: msg.role,
      content: msg.content
    }));
    ...
    if (isNewConsultation) {
      // If starting, the user message is just a trigger. Set a generic message 
      // to remind the AI to follow the greeting protocol first.
      messagesForAI.push({
        role: 'user',
        content: `[START CONSULTATION - Patient's first verbal interaction.]`
      });
    } else {
      // If ongoing, add the patient's actual message to the history.
      messagesForAI.push({ role: 'user', content: userMessageFinal });
    }
    ...
    const userMsg: Message = {
      role: 'user',
      content: isNewConsultation ? userMessageFinal : userMessageFinal, // Store the patient's actual words whether it's the first turn or not.
      timestamp: nowISO(),
      language: conversationLanguageTag,
      natlasData: natlasAnalysis?.success
        ? {
    ...
    // 💡 FIX: When it's the very first message, we only store the AI's greeting (the user's first message is just a trigger to start the session)
    // No, wait. The client sends the user's first audio message when they start speaking.
    // If existingConversation.length === 0, then the user's first message IS the trigger for the AI to give the *greeting*.
    // We should store both the user's first message and the AI's greeting response.
    
    // The previous implementation used an array filter on the client, which masked this slight data discrepancy.
    // Let's ensure we add both messages correctly here.
